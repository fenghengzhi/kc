    ...
    function demandRewardService() {
        ...
        function allclick() {
            ...
            $('.main-con').on('click','.hu-card',function () {
                window.open('demandRewardDetail.html?id='+$(this).attr('id'));
        ...
        function getData() {
            demandRewardService.pageNo = parseInt(commonService.getQuery('pageNo')) || 1;
            let queryCondition = demandRewardService.getQueryCondition();
            ...
            hieknjs.kgLoader({
                ...
                urlData: {
                    pageNo: demandRewardService.pageNo,
                    pageSize: 9
                },
                success: function (data) {
                    if (data && data.rsData && data.rsData.length) {
                        demandRewardService.pageNum=Math.ceil(data.rsCount/9)
                        demandRewardService.pageNo = parseInt(commonService.getQuery('pageNo')) || 1;
                        gentPage();
        ...
        demandRewardService.getQueryCondition = function () {
            let queryCondition = [];
            $('.filter .filter-line').each(function () {
                let _this = $(this);
        ...
        function gentPage() {
            var config = {
                ...
                selector: '#page',
                total: demandRewardService.pageNum,
                showNum: 5,
                current: demandRewardService.pageNo,
                prevNextEnable: false,
                prevNextMultiEnable: true,
                startEndEnable: true,
                callback: function (event, pageNo, data) {
                    demandRewardService.pageNo=pageNo;
